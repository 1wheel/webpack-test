...
function swoopyDrag(){
  ...
  var dispatch$$1 = dispatch('drag');

  var textDrag = drag()
      .on('drag', function(d){
        var x = exports.event.x;
        var y = exports.event.y;
        d.textOffset = [x, y].map(Math.round);

        select(this).call(translate, d.textOffset);

        dispatch$$1.call('drag');
      })
  ...
  var circleDrag = drag()
      .on('drag', function(d){
        ...
        var parentSel = select(this.parentNode);

        var path$$1 = '';
        var points = parentSel.selectAll('circle').data();
        if (points[0].type == 'A'){
          path$$1 = calcCirclePath(points);
        } else{
          points.forEach(function(d){ path$$1 = path$$1 + d.type  + d.pos; });
        }

        parentSel.select('path').attr('d', path$$1).datum().path = path$$1;
        select(this).call(translate, d.pos);
  ...
  var rv = function(sel){
    ...
    annotationSel.selectAll('circle').data(function(d){
      ...
      if (~d.path.indexOf('A')){
        ...
        var pathNode = select(this).select('path').node();
